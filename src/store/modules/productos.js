export const productosModule = {
  ...
  getters: {
    productoDisponiblePorNombre: (state) => (producto) => {
      return state.todosLosProductos.filter((p) => p.precio <= producto || p.nombre.toLowerCase().includes(producto)  || p.categoria.toLowerCase().includes(producto) )
    }
  },
